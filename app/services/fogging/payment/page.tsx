...
export default function FoggingPaymentPage() {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6 items-start">
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-2xl border border-[#E8EDF2] shadow-sm p-6">
            <ServiceSteps steps={[{label:"Citizen Details"},{label:"Request Details"},{label:"Pay"}]} current={3} />
            <h2 className="text-[18px] font-bold mb-5">Amount Details</h2>
            {[["Fogging Charge","₹150"],["Service Fee","₹50"]].map(([l,v])=>(
            ...
            <div className="border-t border-[#E8EDF2] pt-4 flex justify-between"><span className="text-[16px] font-bold">Total Payable</span><span className="text-[22px] font-bold text-[#0A2A76]">₹ 200</span></div>
          </div>
          <div className="bg-white rounded-2xl border border-[#E8EDF2] shadow-sm p-6">
            <h2 className="text-[18px] font-bold mb-5">Choose Payment Method</h2>
            <div className="space-y-3">
              {methods.map(m=>(
                <button key={m} onClick={()=>setMethod(m)} className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 ${method===m?"border-[#0A2A76]":"border-[#E8EDF2]"}`}>
          ...
          <div className="bg-[#0A2A76] rounded-2xl p-6 text-center">
            ...
            <p className="text-[38px] font-bold text-white mb-5">₹200</p>
            <button onClick={()=>router.push("/congratulations?type=fogging&ref=FOG3210&amount=200")} className="w-full bg-[#16A34A] text-white font-bold text-[16px] py-4 rounded-full flex items-center justify-center gap-3">
